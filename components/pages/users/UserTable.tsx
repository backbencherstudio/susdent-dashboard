    ...
    cell: ({ row }) => {
      {
        ...
        <span className="">{row.original.status.charAt(0).toUpperCase() + row.original.status.slice(1)}</span> : 
        <span className="text-[#e70d0d]">{row.original.status.charAt(0).toUpperCase() + row.original.status.slice(1)}</span>
        )
      }
    },
  },
